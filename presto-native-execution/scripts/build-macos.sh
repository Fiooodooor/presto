...
source $SCRIPT_DIR/release-macos-dockerfile/opt/common.sh
...
(
    ...
    if [ "$DOCKER_BUILDKIT" == "0" ]; then
        sed -i 's/--mount=type=ssh//g' "${SCRIPT_DIR}/release-macos-dockerfile/Dockerfile"
    fi
